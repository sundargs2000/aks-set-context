...
function getAKSKubeconfigForARC(azureSessionToken, subscriptionId, managementEndpointUrl) {
    let resourceGroupName = core.getInput('resource-group', { required: true });
    let clusterName = core.getInput('cluster-name', { required: true });
    return new Promise((resolve, reject) => {
        var webRequest = new client_1.WebRequest();
...
function checkClusterType(azureSessionToken, subscriptionId, managementEndpointUrl) {
    let resourceGroupName = core.getInput('resource-group', { required: true });
    let clusterName = core.getInput('cluster-name', { required: true });
    return new Promise((resolve, reject) => {
        var webRequest = new client_1.WebRequest();
...
function getKubeconfig() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        isConnectedCluster = yield checkClusterType(azureSessionToken, subscriptionId, managementEndpointUrl);
        let kubeconfig;
        if (isConnectedCluster)
            kubeconfig = yield getAKSKubeconfigForARC(azureSessionToken, subscriptionId, managementEndpointUrl);
        else
            kubeconfig = yield getAKSKubeconfig(azureSessionToken, subscriptionId, managementEndpointUrl);
        return kubeconfig;
    });
}
function run() {
