...
function getAKSKubeconfig(azureSessionToken, subscriptionId, resourceGroupName, clusterName, managementEndpointUrl) {
    return new Promise((resolve, reject) => {
...
function getAKSContext() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let azureSessionToken = yield getAzureAccessToken(servicePrincipalId, servicePrincipalKey, tenantId, authorityUrl, resourceManagerEndpointUrl);
        let resourceGroupName = core.getInput('resource-group', { required: true });
        let clusterName = core.getInput('cluster-name', { required: true });
        let kubeconfig = yield getAKSKubeconfig(azureSessionToken, subscriptionId, resourceGroupName, clusterName, managementEndpointUrl);
        let aksResourceContext = {
...
function exportKubeConfig(kubeconfig) {
...
function exportResourceContext(aksResourceContext) {
    const runnerTempDirectory = process.env['RUNNER_TEMP']; // Using process.env until the core libs are updated
...
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        let aksContext = yield getAKSContext();
        exportKubeConfig(aksContext.kubeconfig);
        exportResourceContext(aksContext.resourceContext);
    });
}
